...
enum ProviderState: Equatable {
   ...
   case getRunStepsError(message: String)
   ...
   var message: String {
      ...
      case .createMessageDisplayError(let message): return message
      ...
      case .cancelRunSuccess(let message): return message
      case .createRunError(_, _, let message): return message
      case .lastRunStepsError(let message): return message
      case .getRunStepsError(let message): return message
      }
   }
}
...
@Observable class SideMenuConfigurationProvider {
   ...
   private func addOrUpdateAssisantToMapStorage(
   ...
   private func deleteAssistantFromMapStorageWith(
      id: String)
   {
   ...
   private func deleteThreadFromMapStorageWith(
      threadID: String)
   {
   ...
   private func addOrUpdateThreadToMapStorage(
      _ thread: ThreadObject)
   {
      // Check if mapItems contains the threads key and initialize it if not
